...
import { BasketballStatsHeader } from "./BasketballStatsHeader";
...
export const PRHSAABasketballStats = ({ selectedGender, selectedYear }) => {
  const { basketballSchedules, basketballTeams } = PRHSAAstatsFirestoreOverall();
  ...
  return (
    <div className="flex justify-center pt-[50px]">
      <table className="divide-y divide-gray-200 mt-4 text-black">
        ...
        <tbody className="divide-y divide-gray-200">
          {sortedDocuments.map((doc, index) => {
            const teamA = basketballTeams.filter(
              (team) => team.Abbreviation == doc.TeamA
            )[0];
            const teamB = basketballTeams.filter(
              ...
              >
                ...
                <td className="text-center border border-gray-300 py-3">
                  <button
                    ...
                    disabled={doc.Score == undefined || doc.stats.length == 0}
                    onClick={() => {
      ...
      <Modal
        ...
        header={<BasketballStatsHeader selectedDoc={selectedDoc} />}
      >
