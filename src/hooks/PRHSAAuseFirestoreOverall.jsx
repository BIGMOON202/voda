...
const PRHSAAuseFirestoreOverall = () => {
  ...
  const fetchVolleyballStats = async (scheduleId) => {
    const q = query(
      collection(db, `PRHSAAVolleyballSchedules/${scheduleId}/PRHSAAVolleyballStats`)
    );
    const statsData = [];
  ...
  const addSubDocument = async (docId, subcollectionData = {}) => {
    try {
      ...
      const newCityRef = doc(
        collection(db, "PRHSAAVolleyballSchedules", docId, "PRHSAAVolleyballStats")
      );
  ...
  const deleteSubDocument = async (docId, subDocId) => {
    const documentRef = doc(db, col, docId, "PRHSAAVolleyballStats", subDocId);
    try {
...
export { PRHSAAuseFirestoreOverall };
